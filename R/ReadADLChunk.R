...
ReadADLChunk <- function(Source = "", From = "", To = "", FileExtension = "rdata", Directory = F, Iterate = F) {
  ...
  if (FileExtension != "rdata" & FileExtension != "csv")
    catError("Only works with Rdata or CSV files.")
  ...
  dir <- httr::GET(paste0("https://capdevtapdl.azuredatalakestore.net/webhdfs/v1/",
                          data.lake.path, operation), httr::add_headers(Authorization = paste("Bearer",
                                                                                              Data_Lake)))
  fname <- c("The requested month is not available. The available months are:")
  ...
  prefix <- ShowADLPrefix(Source)
  cache <- grep(prefix, cache, value = TRUE)
  if (Directory == FALSE) {

    #Show a Menu when timestamp is not introduced
    if (length(request) == 0) {

      timeAvailable <- gsub(paste(prefix, ".Rdata", sep = "|"),
                            "", fname)
      formatColumns <- unique(c(seq(from = 1, to = length(fname),
                                    5), length(fname)))
      timeAvailable[formatColumns] <- paste0(timeAvailable[formatColumns],
                                             "\n")
      ...
      TAPChunks::catError(paste("", timeAvailable, sep = "- ", collapse = "\t"))

      #answer <- "Wrong"
      #catInfo("Please, enter numbers separated by commas or press 0 to cancel the process:")
      #while (answer[1] == "Wrong") {
      #  answer <- readline("Selection:")
      #  answer <- gsub("\\s", "", answer)
      #  if (answer == 0) {
      #    return("Cancelled")
      #  }
      #  if (length(grep("[^0-9]", gsub(",", "", answer))) ==
      #      1 | answer == "") {
      #    answer <- "Wrong"
      #    catInfo("Re-enter the answer")
      #  }
      #  else {
    ...
    request <- sapply(request, function(x) grep(x, fname,
                                                value = T))
    typefile <- substr(request[1], gregexpr("\\.", request[1])[[1]][1],
                       nchar(request[1]))
    r <- length(request)
    rdesc <- ifelse(r == 1, "chunk", "chunks")
  }
  else {
    summary <- merge(data.table::data.table(ind = fname,
                                            ADL = fname), data.table::data.table(ind = request,
                                                                                 Req = request), by = "ind", all = T)
    summary <- merge(summary, data.table::data.table(ind = cache,
                                                     cache = cache), by = "ind", all = T)
    summary <- summary[, setdiff(names(summary), "ind"),
                       with = F]
    return(summary[, .(`In Data Lake` = ADL, `In Local Cache` = cache)])
  }
  catInfo(paste("You have requested ", r, rdesc))
  a <- length(intersect(request, fname))
  if (a == 0 & r == 1)
    catInfo("That chunk is not in the data lake.")
  else if (a == 0 & r > 1)
    catInfo("None of the requested chunks are in the data lake.")
  else if (a == 1 & r == 1)
    catInfo("The chunk is available in the data lake.")
  else if (a == 1 & r > 1)
    catInfo(paste("Of the", r, "chunks requested,", a, "is available in the data lake."))
  else if (a == r & r > 1)
    catInfo("All the chunks are available in the data lake.")
  else if (a > 1 & r > 1)
    catInfo(paste("Of the", r, "chunks requested,", a, "are available in the data lake."))
  c <- length(intersect(request, cache))
  if (c == 0)
    catInfo("None of your requested chunks are in your cache.")
  else if (a == 1 & c == 1)
    catInfo("Your chunk has been located in the cache.")
  else if (a > 1 & c != 0)
    catInfo(paste("Of the", a, "chunks to be loaded,", c,
                  "will be loaded from cache."))
  CheckRequirements(request)
  result <- NULL
  tempDir <- tempdir()
  result <- list()
  ...
  for (i in request) {
    gc()
    catInfo(sprintf("Loading chunk %s", i))
    assign(gsub(typefile, "", i), ReadADLFile(SpecificPath = data.lake.path, FileName = i,
    ...
    if (attributes(data.lake.path)$source == "Cosmos") {
      ...
      if (!"cosmos_demog_helper" %in% names(TAP_env)) {
        ...
        TAP_env$demog_helper <- rbind(TAP_env$demog_helper,
                                      Cosmos_demog, fill = T)
...
#' @title Reading source chunks from the Azure Data Lake
#' @details This function has been renamed.  See \code{\link{ReadADLChunk}}
#' @keywords internal
#' @export
ADLReadChunk <- function(){
  catWarning("This function has been renamed ReadADLChunk and is deprecated.")
}
