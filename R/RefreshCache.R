...
RefreshCache <-  function() {
  ...
  for (i in source_available) {
    ...
    if (sum(grepl(ShowADLPath(i), files.cache)) != 0) {
      ...
      if (nrow(info.cache) != 0) {
        if (nrow(info.cache) == 1) {
          ...
          catWarning(sprintf("%s: %s file is outdated", i, nrow(info.cache)))
        }
        if (nrow(info.cache) != 1) {

          catWarning(sprintf("%s: %s files are outdated", i, nrow(info.cache)))
        }
        ...
        for (n in info.cache[, file]) {
          ...
          if(grepl(".csv$", n))  ReadADLChunk(i, timestamp,Format = "csv")
          if(grepl(".csv$|.Rdata$", n) == F ) catWarning("wrong format.")

          catSuccess(sprintf("%s file was well updated update", n, nrow(info.cache)))
        }
