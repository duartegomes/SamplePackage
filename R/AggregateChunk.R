...
#' @description Aggregate chunks into a smaller chunks by the metadata selected by the user.
#' @export
...
#' @param Aggregation This is an equation that follows the syntax of R to define the aggregation. The default aggregation is
#' a count of rows.
...
#' @return The function returns the aggregated data chunk.
...
#' @examples AggregateChunk(TestEmailChunk, Aggregation = mean(value), By = "timestamp")
...
AggregateChunk <- function(Data, Filter = 1 == 1, Aggregation = .N, By = NULL, ...) {
  ...
  if (typeof(f1) != "language") {
    TAPChunks:::catError("Syntax error in filter definition.")
  }

  c1 <- substitute(Aggregation)
  if (deparse(c1) != ".N") {
    if (typeof(c1) != "language" | grepl(":=", deparse(c1))) {
      TAPChunks:::catError("Syntax error in aggregation definition.")
    }
  }
  ...
  e <- strsplit(e, ":")[[1]][2]
  e <- gsub("\n", "", e)
  if (!exists("tb")) TAPChunks:::catError(paste0("Syntax error in the group by definition:", e, "."))
  if (!(tb == "character" | tb == "NULL")) TAPChunks:::catError("Syntax error in the group by definition.")
  ...
  TAPChunks:::catError(paste0("Syntax error:", e, "."))
}
